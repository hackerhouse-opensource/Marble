...
#include <stdlib.h>
#include <time.h>
#include "MBL_FORLOOP_BUMP6.h"

MBL_FORLOOP_BUMP6::MBL_FORLOOP_BUMP6(void)
{
	cKeyLiteral = NULL;
	cWKeyLiteral = NULL;
	...
	//create key literals
	CreateStringLiteralA(cKey, 6, cKeyLiteral);
	CreateStringLiteralW(wcKey, 6, cWKeyLiteral);
}
...
int MBL_FORLOOP_BUMP6::ScrambleW(wchar_t *wcToScramble, unsigned int iNumOfChars)
{
	if (wcToScramble == NULL) return 0;
	for (int i = 0; i < iNumOfChars; i++)
		wcToScramble[i] += wcKey[i % 6];

	return 1;
}

int MBL_FORLOOP_BUMP6::ScrambleA(char *cToScramble, unsigned int iNumOfChars)
{
	if (cToScramble == NULL) return 0;
	for (int i = 0; i < iNumOfChars; i++)
		cToScramble[i] += cKey[i % 6];

	return 1;
}

int MBL_FORLOOP_BUMP6::GenerateInsertA(char *cVarName, char *cStringLiteral, unsigned int iNumOfChars, char *&cInsert)
{
...
int MBL_FORLOOP_BUMP6::GenerateInsertW(char *cVarName, char *cStringLiteral, unsigned int iNumOfChars, char *&cInsert)
{
	if (cVarName == NULL || cStringLiteral == NULL)
		return 0;
	cInsert = NULL;
